...
function scoreBadgeClass(score: number): string {
  if (score >= 80) return 'bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20';
  if (score >= 50) return 'bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 border border-yellow-500/20';
  ...
  return (
    <section className="space-y-4">
        ...
        <CardContent className="p-0">
          <ul className="divide-y divide-border">
            {CHECK_ORDER.map((key) => {
              ...
              return (
                <li key={key} className="flex items-center gap-3 hover:bg-muted/50 rounded-lg px-3 py-2 transition-colors">
                  {passing ? (
      ...
      {llmSignals && llmSignals.length > 0 && (
        <div className="bg-secondary border border-indigo-500/20 rounded-xl p-4">
